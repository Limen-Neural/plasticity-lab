...
- CI: Build & Test matrix on `ubuntu-latest`, `macos-latest`, and
  `windows-latest` (`fail-fast: false`). `cargo fmt --check`, musl
  `cargo-deny`, rustdoc, and tarpaulin/Codecov stay Linux-only so coverage
  is not duplicated across OS cells.
