...
- `Cargo.toml` package metadata: `repository`, `readme`, `keywords`,
  `categories`, and an explicit package-file allowlist (#68)
...
- `tracing` from `[dependencies]` (#67, #68): it had no code in `src/` using
  it. `serde_json` moves from
...
- Dependencies now resolve from crates.io: `neuromod = "0.6.0"` and optional
  `limbic-critic = "0.3.0"`. The `critic` feature remains opt-in.
