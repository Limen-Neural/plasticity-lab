...
- MSRV build/test — substitute `Cargo.toml`'s `rust-version` (the same value `.github/workflows/ci.yml`'s `msrv` job hardcodes): `cargo +<version> build --locked --all-features && cargo +<version> test --locked --all-features`

`rust-toolchain.toml` pins the toolchain for local dev/rustup auto-detection only — neither CI job derives from it. `.github/workflows/ci.yml`'s `validate` and `msrv` jobs each hardcode their own explicit `toolchain:` version instead. This is deliberate for `msrv` (see its own comment in the workflow): it stays pinned to the declared MSRV even if `rust-toolchain.toml`'s `channel` advances, so the job keeps catching real MSRV violations regardless of what contributors run locally. Two separate rules, don't conflate them: raising the actual MSRV means bumping `Cargo.toml`'s `rust-version` and the `msrv` job's hardcoded toolchain together (these two must always match each other); advancing the day-to-day dev toolchain means bumping `rust-toolchain.toml`'s `channel` (and, by convention so far, the `validate` job's pin) on its own — that does *not* require touching the `msrv` job.
